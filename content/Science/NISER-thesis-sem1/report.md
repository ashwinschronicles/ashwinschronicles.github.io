...
![A schematic representation of RCSJ model. Here $I$ is the current through the device, $I_{c}$ is the current through the capacitor $I_{J}$ is the current through the Josephson Junction, $I_{R}$ is the current through the resistance]({attach}Photos/RCSJ.webp){#fig:Schematic-of-RCSJ}
...
![Image of the mask that was used for lithography]({attach}Photos/pattern.webp)#{#fig:mask}

![Image of the mask aligner that was used for lithography]({attach}Photos/maskAligner.webp)#{#fig:mask-1}
...
![The different profiles achieved with (a) a single layer of resist, (b) resist soaked in chlorobenzene for few seconds and (c) a bilayer of electron sensitive resist [@undercutImage]]({attach}Photos/photo-resist.webp){#fig:The-different-profiles}
...
![Image taken from optical microscope of the sample after the photolithography process ]({attach}Photos/litho1.webp)#{#fig:Litho1 }
...
![Image taken from optical microscope of the sample after deposition and liftoff process ]({attach}Photos/Litho2.webp)#{#fig:Litho2 }
...
![The DC-magnetron sputtering setup present in superconductivity lab, NISER ]({attach}Photos/Sputter-Coater.webp)#{#fig:The-DC-magnetron-sputtering }
...
|-----------|-----------------------------------------------------------------------------------|------------------------------------|-----------------|--------------------|--------------------------------|--------------------------------|
|   Nb      |          1.08 x 1            0                   - 2                m b a r       |   30 mm                            |   55 W          |   20SCCM           |   25.2 degree                  |   0.06 deg/min for 155nm       |
|   Cu      |          1.08 x 1            0                   - 2                m b a r       |   30 mm                            |   50 W          |   20SCCM           |   10.8 degree                  |   0.1 deg/min for 100nm        |
|   Pt      |          1.08 x 1            0                   - 2                m b a r       |   30 mm                            |   50 W          |   20SCCM           |   10.8 degree                  |   1 deg/min for 20nm           |
...
![The shadow deposition technique for Josephson Junction fabrication [@JJfab]]({attach}Photos/shadow-dep.webp){#fig:The-shadow-deposition }
...
![Alignment of SEM gun (grey) and FIB gun (brown) axis at the surface of the sample (purple) mounted on the stage (silver) ]({attach}Photos/secondRenderY.webp)#{#fig:Alignment-of-SEM }
...
![Energy Dispersive Spectroscopy (EDAX) of the multiple vertical cuts done in order to optimise the FIB cutting depth ]({attach}Photos/Edax.webp)#{#fig:Edax }
...
![Image taken from optical microscope of the sample after milling with FIB, the devices are encircled]({attach}Photos/Litho-3.webp)#{#fig:Litho3 }
...
![image]({attach}Photos/wedge tip.webp){}![image]({attach}Photos/wedge tip2.webp)
...
![The steps involved in making a bond from the puck to the contact pads on the sample [@wirebonder]]({attach}Photos/Wedge bonding process.webp){#fig:wirebondersteps }
...
![Image of a bond site which was done by wedge bonding tool. [@wirebonder]]({attach}Photos/wedgeboding-surface.webp){#fig:wirebondersteps-1 }
...
![Image of the PPMS system present at Superconductivity lab NISER]({attach}Photos/PPMS.webp)#{#fig:PPMS }
...
![RT graph for a Cu(100nm)Nb(150nm) Josephson junction. The inset shows an SEM image of the measured JJ ]({attach}Photos/JJ.webp)#{#fig:RT-graph-for }
...
![IV graph for a Cu(100nm)Nb(150nm) Josephson junction ]({attach}Photos/IV Pt20 CuNbJJ 2k.webp)#{#fig:IV-graph-for }
...
![A sample graph of $dI/dV$ and $d^{2}I/d^{2}V$ for a I vs V curve measured on a Josephson junction. The $I_{c}$ extracted from the graph is 140$\mu A$ ]({attach}Photos/samplePlot.webp)#{#fig:findIc }

The code for the python script is available [here](https://github.com/iamashwin99/JJ-Ic-finder) and a web app based on the same is hosted at [jj-ic-finder.streamlit.app](https://jj-ic-finder.streamlit.app/)
...
![V H graph for a Cu(100nm)Nb(150nm) SQUID, SQUID oscillations and Fraunhofer pattern can be seen]({attach}Photos/Squid VH.webp)#{#fig:V-H-graphSQUID }

![Zoomed in graph of V H for a Cu(100nm)Nb(150nm) SQUID, The SQUID oscillations can be seen clearly]({attach}Photos/Squid-Oscilations.webp)#{#fig:V-H-graphSQUID-1 }

![Magnetoresistance of the patterned Nb/Cu Josephson junction device in low magnetic fields for different values of junction currents]({attach}Photos/VH CuNbJJ 2k.webp)#{#fig:Magnetoresistance-of-the }

![Magnetoresistance of the patterned Nb/Cu Josephson junction device in low magnetic fields]({attach}Photos/JJ VH.webp)#{#fig:Magnetoresistance-of-the-1 }
